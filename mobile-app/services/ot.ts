import { CompleteTransferResponse, ICompleteTransferBody, InitiateTransferResponse } from "../models/api";
...
// temporary URL, is generated by ngrok on every start-up
const API_URL = "https://938b481de507.ngrok.io/api";
...
export const verifyLocations = async (locations: ILocation[]): Promise<ILocation[]> => {
  const riskyLocations: ILocation[] = [];
  ...
  for (const sectorIdentifier of Object.keys(locationGroup)) {
    ...
    }) as CompleteTransferResponse;

    for (const [index, response] of results.entries()) {
      receiver.setChoice(locationGroup[sectorIdentifier].valueArr[index]);
      const retrievedValue = receiver.interpretSenderResponse(response);

      if (retrievedValue === "1") {
        riskyLocations.push(locationGroup[sectorIdentifier].locationArr[index]);
      }
    }
  }
  // console.log(riskyLocations);
  return riskyLocations;
};
