...
class OTSender {
   ...
   */
  constructor(p: ISafePrime, q: ISafePrime, messageBitCount: number, messages: string[], y?: bigint) {
    this.p = p;
    this.q = q;
    ...
    this.y = y ? y : generateQNRModRSA(this.n);
  }
  ...
  prepareInitialValues(): IOTSenderInitialValues {
    ...
    for(const message of this.messages) {
      ...
      for(const currentBit of message) {
        ...
        const z = ((c ** 2n) * (this.y ** BigInt(currentBit))) % this.n.value;
        encoding.push(z);
      }
